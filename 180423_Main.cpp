...
#define iWidth 4608
#define iHeight 3456
#define CROP_W_S 1045 
#define CROP_H_S 1345 
#define CROP_W_E 1145
#define CROP_H_E 1445 
...
void DropYUVFile(char * argv)
{
	...
	int CROP_SIZE = (CROP_H_E - CROP_H_S) * (CROP_W_E - CROP_W_S);
	int CROP_H = CROP_H_E - CROP_H_S;
	int CROP_W = CROP_W_E - CROP_W_S;
	...
	if (file1.is_open() & file2.is_open() & file3.is_open() & file4.is_open())
	{
		...
		for (j = 0; j < CROP_H; j++)
		{
			for (i = 0; i < CROP_W; i++)
			{
				Pixel_Y_Croped[k++] = Pixel_Y[((CROP_H_S + j) * iWidth) + CROP_W_S + i];
			}
		}
		...
		cout << argv[1] << " Sharpening has been adjusted" << endl;
